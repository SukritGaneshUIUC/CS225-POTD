...
bool BTreeNode::rotateRight(unsigned idx, unsigned order) {
    ...
    // steal an element from left
    // must be largest element in left child (back)
    int stolenElement = prev->elements_.at(prev->elements_.size() - 1);
    prev->elements_.assign(prev->elements_.begin(), prev->elements_.begin() + prev->elements_.size() - 1);

    // must add current element to current child (elements[idx], child[idx])
    // add to front of the child's elements
    children_.at(idx)->elements_.insert(children_.at(idx)->elements_.begin(), elements_.at(idx - 1));

    // current element must be stolen element
    elements_[idx - 1] = stolenElement;
